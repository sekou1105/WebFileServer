...
void AcceptConn::process(){
    ...
    int accFd = accept(m_listenFd, (sockaddr*)&m_clientAddr, &m_clientAddrLen);
    if(accFd == -1){
        LOG_ERROR("接受新连接失败");
        return;
    }
    ...
    while(1) {
        ...
        if(recvLen == -1){
            if(errno != EAGAIN){    // 如果不是缓冲区为空，设置状态为错误，并退出循环
                requestStatus[m_clientFd].m_status = HANDLE_ERROR;
                LOG_ERROR("接收数据时返回 -1 (errno = %d)", errno);
                break;
            }
        ...
        requestStatus[m_clientFd].m_recvMsg.append(buf, recvLen);
        ...
        if(requestStatus[m_clientFd].m_status == HANDLE_INIT){

            endIndex = requestStatus[m_clientFd].m_recvMsg.find("\r\n");       // 查找请求行的结束边界

            if(endIndex != std::string::npos){
                // 保存请求行  
                requestStatus[m_clientFd].setRequestLine(requestStatus[m_clientFd].m_recvMsg.substr(0, endIndex + 2) ); // std::cout << requestStatus[m_clientFd].recvMsg.substr(0, endIndex + 2);
                requestStatus[m_clientFd].m_recvMsg.erase(0, endIndex + 2);    // 删除收到的数据中的请求行
                requestStatus[m_clientFd].m_status = HANDLE_HEAD;              // 将状态设置为处理消息首部
                LOG_INFO("处理客户端 %d 的请求行完成", m_clientFd);
            }
        ...
        if(requestStatus[m_clientFd].m_status == HANDLE_HEAD){
            ...
            while(1){
                ...
                curLine = requestStatus[m_clientFd].m_recvMsg.substr(0, endIndex + 2);  // 将该行的内容取出
                requestStatus[m_clientFd].m_recvMsg.erase(0, endIndex + 2);             // 删除收到的数据中的该行数据

                if(curLine == "\r\n"){
                    requestStatus[m_clientFd].m_status = HANDLE_BODY;                                       // 如果是空行，将状态修改为等待解析消息体
                    if(requestStatus[m_clientFd].m_msgHeader["Content-Type"] == "multipart/form-data"){     // 如果接收的是文件，设置消息体中文件的处理状态
                        requestStatus[m_clientFd].fileMsgStatus = FILE_BEGIN_FLAG;
                    }
                    //std::cout << outHead("info") << "处理客户端 " << m_clientFd << " 的消息首部完成" << std::endl;
                    if(requestStatus[m_clientFd].m_requestMethod == "POST"){
                        LOG_INFO("客户端 %d 发送 POST 请求，开始处理请求体", m_clientFd);
                    }
                    break;                                                                                // 退出首部字段循环
                }
        ...
        if(requestStatus[m_clientFd].m_status == HANDLE_BODY){
            // GET 操作时表示请求数据，将请求的资源路径交给 HandleSend 事件处理
            if(requestStatus[m_clientFd].m_requestMethod == "GET"){
                // 设置响应消息的资源路径，在 HandleSend 中根据请求资源构建整个响应消息并发送
                responseStatus[m_clientFd].m_bodyFileName = requestStatus[m_clientFd].m_requestResource;
                ...
                LOG_INFO("客户端 %d 发送 GET 请求，已将请求资源构成 Response 写事件等待发送数据", m_clientFd); 
                break;
            }
            ...
            if(requestStatus[m_clientFd].m_requestMethod == "POST"){
                ...
                std::string::size_type beginSize = requestStatus[m_clientFd].m_recvMsg.size();
                if(requestStatus[m_clientFd].m_msgHeader["Content-Type"] == "multipart/form-data"){  // 如果发送的是文件
                    ...
                    if(requestStatus[m_clientFd].fileMsgStatus == FILE_BEGIN_FLAG){
                        ...
                        // 当前状态下，\r\n 前的数据必然是文件信息开始的标志
                        if(endIndex != std::string::npos){
                            std::string flagStr = requestStatus[m_clientFd].m_recvMsg.substr(0, endIndex);
                    ...
                    if(requestStatus[m_clientFd].fileMsgStatus == FILE_HEAD){
                        ...
                        while(1){
                            ...
                            endIndex = requestStatus[m_clientFd].m_recvMsg.find("\r\n");
                            if(endIndex != std::string::npos){
                                strLine = requestStatus[m_clientFd].m_recvMsg.substr(0, endIndex + 2);  // 获取这一行的数据信息
                                requestStatus[m_clientFd].m_recvMsg.erase(0, endIndex + 2);             // 删除这一行信息
                    ...
                    if(requestStatus[m_clientFd].fileMsgStatus == FILE_CONTENT){
                        ...
                        while(1){
                            int saveLen = requestStatus[m_clientFd].m_recvMsg.size();        // 该变量用来保存 根据\r的位置决定向文件中写入多少字符，初始为所有字符长度
                            if(saveLen == 0){                                              // 长度为空时退出循环，等待接收到数据时再处理
                                break;
                            }
                            // 在剩余的字符中搜索标志 \r
                            endIndex = requestStatus[m_clientFd].m_recvMsg.find('\r');
                                        
                            if(endIndex != std::string::npos){   // 如果有\r，后面有可能是文件结束标识
                                // 首先判断 \r 后的数据是否满足结束标识的长度，是否大于等于 sizeof(\r\n + "--" + boundary + "--" + \r\n)
                                int boundarySecLen = requestStatus[m_clientFd].m_msgHeader["boundary"].size() + 8;
                                if(requestStatus[m_clientFd].m_recvMsg.size() - endIndex >= boundarySecLen){
                                    // 判断后面这部分数据是否为结束边界"\r\n"
                                    if(requestStatus[m_clientFd].m_recvMsg.substr(endIndex, boundarySecLen) ==
                                        ...
                                        endIndex = requestStatus[m_clientFd].m_recvMsg.find('\r', endIndex + 1);
                                        if(endIndex != std::string::npos){
                                            saveLen = endIndex;
                                        }
                                    }
                            ...
                            ofs.write(requestStatus[m_clientFd].m_recvMsg.c_str(), saveLen);
                            requestStatus[m_clientFd].m_recvMsg.erase(0, saveLen);
                        }
                        ofs.close();
                    }
...
void HandleSend::getFileListJson(std::string &fileListJson, const char *filedir) {
    // 将指定目录内的所有文件保存到 fileVec 中
    std::vector<std::string> fileVec;
    getFileVec(filedir, fileVec);

    //构建JSON
    fileListJson = "{\"code\":200,\"msg\":\"success\",\"data\":[";
    for(size_t i = 0; i < fileVec.size(); ++i) {
        fileListJson += "\"" + fileVec[i] + "\"";
        if (i < fileVec.size() - 1) {
            fileListJson += ",";
        }
    }
...
std::string HandleSend::getStatusLine(const std::string &httpVersion, const std::string &statusCode, const std::string &statusDes){
    ...
    // 记录状态行相关的参数
    responseStatus[m_clientFd].m_responseHttpVersion = httpVersion;
    responseStatus[m_clientFd].m_responseStatusCode = statusCode;
    responseStatus[m_clientFd].m_responseStatusDes = statusDes;
    // 构建状态行
...
std::string HandleSend::getMessageHeader(const std::string contentLength, const std::string contentType, \
...
void HandleSend::process() {
    ...
    if(responseStatus[m_clientFd].m_status == HANDLE_INIT){
        // 首先分离操作方法和文件
        std::string opera, filename;
        if(responseStatus[m_clientFd].m_bodyFileName == "/"){
            ...
            // 文件名的查找中间 / 的索引
            int i = 1;
            while(i < responseStatus[m_clientFd].m_bodyFileName.size() && responseStatus[m_clientFd].m_bodyFileName[i] != '/'){
                ++i;
            }
            // 检查是否包含操作和对应的文件名，如果不满足 操作+文件名 的格式，设置为重定向操作，将页面重定向到文件列表页面
            if(i < responseStatus[m_clientFd].m_bodyFileName.size() - 1){
                opera = responseStatus[m_clientFd].m_bodyFileName.substr(1, i - 1);
                filename = responseStatus[m_clientFd].m_bodyFileName.substr(i+1);
            }else{
                opera = "redirect";
            }
        ...
        if(opera == "/"){                   //如果是根目录，返回文件夹中的所有文件名字
            ...
            // 先创建响应体对应的json数据
            getFileListJson(responseStatus[m_clientFd].m_msgBody);
            // 记录页面的字节个数，即消息体长度
            responseStatus[m_clientFd].m_msgBodyLen = responseStatus[m_clientFd].m_msgBody.size();


            // 根据消息体的数据长度添加头部信息
            //responseStatus[m_clientFd].beforeBodyMsg += getMessageHeader(std::to_string(responseStatus[m_clientFd].msgBodyLen), "html");
            responseStatus[m_clientFd].m_beforeBodyMsg += getMessageHeader(std::to_string(responseStatus[m_clientFd].m_msgBodyLen), "json");
            // 加入空行
            responseStatus[m_clientFd].m_beforeBodyMsg += "\r\n";

            responseStatus[m_clientFd].m_beforeBodyMsgLen = responseStatus[m_clientFd].m_beforeBodyMsg.size();
            ...
            if(responseStatus[m_clientFd].m_fileMsgFd == -1){                  // 文件打开失败时，退出当前函数（避免下面关闭文件造成错误），并重置写事件，在下次进入时回复重定向报文
                ...
                fstat(responseStatus[m_clientFd].m_fileMsgFd, &fileStat);
                
                // 获取文件长度，作为消息体长度
                responseStatus[m_clientFd].m_msgBodyLen = fileStat.st_size;
                
                // 根据消息体构建消息首部
                responseStatus[m_clientFd].m_beforeBodyMsg += getMessageHeader(std::to_string(responseStatus[m_clientFd].m_msgBodyLen), "file", std::to_string(responseStatus[m_clientFd].m_msgBodyLen - 1));
                // 加入空行
                responseStatus[m_clientFd].m_beforeBodyMsg += "\r\n";
                responseStatus[m_clientFd].m_beforeBodyMsgLen = responseStatus[m_clientFd].m_beforeBodyMsg.size();
                
                // 设置标识，转换到发送数据的状态
                responseStatus[m_clientFd].m_bodyType = FILE_TYPE;      // 设置消息体的类型
            ...
            // 添加状态行
            responseStatus[m_clientFd].m_beforeBodyMsg = getStatusLine("HTTP/1.1", "302", "Moved Temporarily");

            // 构建重定向的消息首部
            responseStatus[m_clientFd].m_beforeBodyMsg += getMessageHeader("0", "html", "/", "");
    ...
    while(1){
        long long sentLen = 0;
        // 发送响应消息头
        if(responseStatus[m_clientFd].m_status == HANDLE_HEAD){
            // 开始发送消息体之前的所有数据
            sentLen = responseStatus[m_clientFd].m_curStatusHasSendLen;
            sentLen = send(m_clientFd, responseStatus[m_clientFd].m_beforeBodyMsg.c_str() + sentLen, responseStatus[m_clientFd].m_beforeBodyMsgLen - sentLen, 0);
            if(sentLen == -1) {
                if(errno != EAGAIN){
            ...
            responseStatus[m_clientFd].m_curStatusHasSendLen += sentLen;
        ...
        // 发送响应消息体
        if(responseStatus[m_clientFd].m_status == HANDLE_BODY){
            // 根据发送数据的类型执行特定的发送操作
            if(responseStatus[m_clientFd].m_bodyType == JSON_TYPE){
                // 消息体为 JSON 时的发送方法
                sentLen = responseStatus[m_clientFd].m_curStatusHasSendLen;
                sentLen = send(m_clientFd, responseStatus[m_clientFd].m_msgBody.c_str() + sentLen, responseStatus[m_clientFd].m_msgBodyLen - sentLen, 0);
                if(sentLen == -1){
                    if(errno != EAGAIN){
                        requestStatus[m_clientFd].m_status = HANDLE_ERROR;
            ...
            }else if(responseStatus[m_clientFd].m_bodyType == FILE_TYPE){
                // 消息体是文件时的发送方法
                ...
                sentLen = sendfile(m_clientFd, responseStatus[m_clientFd].m_fileMsgFd, (off_t *)&sentLen, responseStatus[m_clientFd].m_msgBodyLen - sentLen);
                if(sentLen == -1){
                    if(errno != EAGAIN){
                        // 如果不是缓冲区满，设置发送失败状态
                        requestStatus[m_clientFd].m_status = HANDLE_ERROR;
            ...
            }else if(responseStatus[m_clientFd].m_bodyType == EMPTY_TYPE){
        ...
        if(responseStatus[m_clientFd].m_status == HANDLE_ERROR){    // 如果是出错状态，退出 while 处理
            break;
        }
    }

    // 判断发送最终状态执行特定的操作
    if(responseStatus[m_clientFd].m_status == HADNLE_COMPLETE){
        // 完成发送数据后删除该响应
    ...
    // 处理成功或非文件打开失败时需要关闭文件
    if(responseStatus[m_clientFd].m_bodyType == FILE_TYPE){
        close(responseStatus[m_clientFd].m_fileMsgFd);
    }
